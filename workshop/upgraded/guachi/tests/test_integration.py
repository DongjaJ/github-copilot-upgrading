...
class TestIntegration(unittest.TestCase):
	...
	def test_access_mapped_configs_empty_dict(self):
		...
		foo.set_config({})
		self.assertEqual(foo(), self.mapped_defaults)
		self.assertEqual(foo.path, '/tmp/guachi/guachi.db')
		self.assertEqual(foo.get_ini_options(), self.mapped_options)
		self.assertEqual(foo.get_default_options(), self.mapped_defaults)
